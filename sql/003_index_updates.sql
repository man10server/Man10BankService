...
-- cheque_tbl: add helper indexes (MySQL does not support CREATE INDEX IF NOT EXISTS)
DROP INDEX IF EXISTS cheque_tbl_uuid_index   ON cheque_tbl;
DROP INDEX IF EXISTS cheque_tbl_player_index ON cheque_tbl;
...
DROP INDEX IF EXISTS loan_table_lend_player_uuid_index ON loan_table;
...
DROP INDEX IF EXISTS server_estate_history_date_index ON server_estate_history;
CREATE INDEX server_estate_history_date_index ON server_estate_history (date);

-- server_loan_tbl: add player index
DROP INDEX IF EXISTS server_loan_tbl_player_index ON server_loan_tbl;
